#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
#include "fq_arith.h"
#include "permutation.h"
#include "codes.h"
#include "SPECK.h"
#include "parameters.h"
#include "rng.h"
#include "api.h"
#include "transpose.h"
#include "utils.h"

#define GRN "\e[0;32m"
#define WHT "\e[0;37m"

#ifdef N_pad
#define NN N_pad
#else
#define NN N
#endif

/// taken from the nist package.
void fprintBstr(FILE *fp, char *S, unsigned char *A, unsigned long long L) {
    unsigned long long  i;
    fprintf(fp, "%s", S);
    for ( i=0; i<L; i++ )
        fprintf(fp, "%02X", A[i]);
    if ( L == 0 )
        fprintf(fp, "00");
    fprintf(fp, "\n");
}

/* Exhaustive testing of inverses mod Q */
void inverse_mod_tester(void){
    uint32_t value[Q-1];
    uint32_t inverse[Q-1];
    for(uint32_t i=1; i <= Q-1; i++){
        value[i-1] = i;
        inverse[i-1] = fq_inv(i);
    }
    int all_ok = 1;
    for(uint32_t i=1; i <= Q-1; i++){
        if((value[i-1]*inverse[i-1]) % Q !=1){
           printf("%u*%u=%u\n",
                  value[i-1],
                  inverse[i-1],
                  (value[i-1]*inverse[i-1])%Q);
           all_ok = 0;
        }
    }
    if (all_ok){
        puts("All inverses on F_q ok");
    }
}


/*
 *
 */
void info(void){
    fprintf(stderr,"Code parameters: n= %d, k= %d, q=%d\n", N,K,Q);
    fprintf(stderr,"num. keypairs = %d\n",NUM_KEYPAIRS);
    fprintf(stderr,"Private key: %luB\n", sizeof(speck_prikey_t));
    fprintf(stderr,"Public key %luB, %.3f kiB\n", sizeof(speck_pubkey_t), ((float) sizeof(speck_pubkey_t))/1024);
    fprintf(stderr,"Max Signature size: %luB, %.3f kiB\n", sizeof(speck_sign_t), ((float) sizeof(speck_sign_t))/1024);

}


#define NUMBER_OF_TESTS 10
#define MLEN 160

int SPECK_sign_verify_test_KAT(void) {
    //unsigned char m[] = {0xD8,0x1C,0x4D,0x8D,0x73,0x4F,0xCB,0xFB,0xEA,0xDE,0x3D,0x3F,0x8A,0x03,0x9F,0xAA,0x2A,0x2C,0x99,0x57,0xE8,0x35,0xAD,0x55,0xB2,0x2E,0x75,0xBF,0x57,0xBB,0x55,0x6A,0xC8};
	//uint8_t seed[] = {0x06,0x15,0x50,0x23,0x4D,0x15,0x8C,0x5E,0xC9,0x55,0x95,0xFE,0x04,0xEF,0x7A,0x25,0x76,0x7F,0x2E,0x24,0xCC,0x2B,0xC4,0x79,0xD0,0x9D,0x86,0xDC,0x9A,0xBC,0xFD,0xE7,0x05,0x6A,0x8C,0x26,0x6F,0x9E,0xF9,0x7E,0xD0,0x85,0x41,0xDB,0xD2,0xE1,0xFF,0xA1};
    // uint8_t seed[] = {0xF1,0x90,0x2A,0x78,0x15,0xF3,0x7B,0xC7,0xF5,0x80,0x2D,0x8C,0xBC,0xE5,0xB4,0x8D,0x82,0xEB,0x85,0x69,0x17,0x18,0x06,0x2B,0xFB,0x84,0xD8,0xC0,0x6A,0xA4,0x1D,0x6E,0x90,0x39,0xB0,0xA1,0x07,0x24,0x5D,0xAF,0xA4,0xEC,0x10,0x9A,0x57,0x33,0x29,0x14};
    // uint8_t m[] = {};
    uint8_t seed[] ={0x83,0xC6,0x53,0x70,0x8F,0xAF,0x3E,0x5F,0x6F,0xBC,0x9D,0xFB,0xE6,0xFB,0x5E,0x83,0xE5,0x72,0xA7,0x68,0x86,0x45,0xD7,0x5D,0x2C,0x48,0x35,0xB2,0x86,0x95,0xDE,0xA4,0xBD,0x70,0x93,0x74,0x0D,0x0F,0xF4,0x32,0x37,0x35,0x4E,0xAD,0x1C,0x97,0x8B,0xC2};
    uint8_t m[] ={};

    //for (uint32_t i=0; i<48; i++) {
    //    seed[i] = i;
	//}

    init_randombytes(seed, 48);
    initialize_csprng(&platform_csprng_state,
                      (const unsigned char *)seed,
                      48);

    const uint32_t mlen = sizeof(m);
    unsigned long long smlen = 0, mlen1;
    unsigned char *m1 = (unsigned char *)calloc(mlen+CRYPTO_BYTES, sizeof(unsigned char));
    unsigned char *sm = (unsigned char *)calloc(mlen+CRYPTO_BYTES, sizeof(unsigned char));
    unsigned char pk[CRYPTO_PUBLICKEYBYTES] = {0}, sk[CRYPTO_SECRETKEYBYTES] = {0};

    int ret_val;
    if ((ret_val = crypto_sign_keypair(pk, sk)) != 0) {
        printf("crypto_sign_keypair returned <%d>\n", ret_val);
        return -1;
    }

    fprintBstr(stdout, "pk = ", pk, CRYPTO_PUBLICKEYBYTES);
    fprintBstr(stdout, "sk = ", sk, CRYPTO_SECRETKEYBYTES);


    if ( (ret_val = crypto_sign(sm, &smlen, m, mlen, sk, pk)) != 0) {
        printf("crypto_sign returned <%d>\n", ret_val);
        return -1;
    }

   fprintBstr(stdout, "sm = ", sm, smlen);

    if ( (ret_val = crypto_sign_open(m1, &mlen1, sm, smlen, pk)) != 0) {
        printf("crypto_sign_open returned <%d>\n", ret_val);
        return -1;
    }
    free(m1);
    free(sm);
    printf("all good\n");
    return 0;
}

int check_antiorthogonal(const FQ_ELEM A[K][N_K_pad]){
    //Check decompressed G antiorthogonality
    FQ_ELEM row_by_row;
    for(uint16_t i=0; i < K; i++){
        for(uint16_t j=0; j < K; j++){
            row_by_row = 0;
            for(uint16_t k=0; k<N-K; k++){
                row_by_row = fq_add(row_by_row,fq_mul(A[i][k],A[j][k]));
            }
            if(row_by_row != 0 && i!=j){
                printf("FAIL! Some rows %u and %u are not orthogonal!\n", i,j);
                printf("Row by row = %u\n", row_by_row);
                inner_prod(&row_by_row,A[i],A[j]);
                printf("Row by row = %u\n", row_by_row);
            }else if(row_by_row != Q-1 && i==j){
                printf("FAIL! Row %u is not self-antiorthogonal\n", i);
                printf("Row by row = %u\n", row_by_row);
                inner_prod(&row_by_row,A[i],A[j]);
                printf("Row by row = %u\n", row_by_row);
            }
        }
    }
    return 0;
}


void test_sample_antiorthogonal(){
    unsigned char compressed_sk[PRIVATE_KEY_SEED_LENGTH_BYTES];
    randombytes(compressed_sk, PRIVATE_KEY_SEED_LENGTH_BYTES);
    SHAKE_STATE_STRUCT sk_shake_state;
    initialize_csprng(&sk_shake_state, compressed_sk, PRIVATE_KEY_SEED_LENGTH_BYTES);
    unsigned char G_seed[SEED_LENGTH_BYTES];
    csprng_randombytes(G_seed, SEED_LENGTH_BYTES, &sk_shake_state);

    FQ_ELEM A[K][N_K_pad] = {0};
    sample_antiorthogonal(A,G_seed);

    check_antiorthogonal(A);
}

void test_c1s_compression(void){

    FQ_ELEM u[K];
    FQ_ELEM c1s[128][K_pad] = {0};
    for(int i=0; i<128; i++){
        rand_range_q_elements(u,K);
        for(int j=0; j<K; j++){
            c1s[i][j] = u[j];
        }
    }

    FQ_ELEM compressed_c1s[((BITS_TO_REPRESENT(Q)*(K)*128 + 7)/8)];
    compress_c1s(compressed_c1s,128,c1s);

    FQ_ELEM recovered_c1s[128][K_pad] = {0};
    expand_c1s(128,recovered_c1s,compressed_c1s);
    for(int i=0; i<128; i++){
        for(int j=0; j<K; j++){
            if(c1s[i][j] != recovered_c1s[i][j]){
                printf("c1s do not coincide!\n");
            }
        }
    }

}

void test_rref(void){
    for(int run = 0; run < 100; run++){
        unsigned char compressed_sk[PRIVATE_KEY_SEED_LENGTH_BYTES];
        randombytes(compressed_sk, PRIVATE_KEY_SEED_LENGTH_BYTES);
        SHAKE_STATE_STRUCT sk_shake_state;
        initialize_csprng(&sk_shake_state, compressed_sk, PRIVATE_KEY_SEED_LENGTH_BYTES);
        unsigned char G_seed[SEED_LENGTH_BYTES];
        csprng_randombytes(G_seed, SEED_LENGTH_BYTES, &sk_shake_state);
        rref_generator_mat_t G;

        for(int i=0; i<K; i++){
            memset(G.values[i],0,sizeof(FQ_ELEM)*N_K_pad);
        }

        sample_generator(&G,G_seed);

        uint16_t compressed[SPECK_RREF_MAT_PACKEDBYTES] = {0};

        // First let's test compression
        compress_rref_speck_non_IS(compressed, &G);
        
        rref_generator_mat_t G_expanded;

        for(int i=0; i<K; i++){
            memset(G_expanded.values[i],0,sizeof(FQ_ELEM)*N_K_pad);
        }

        expand_to_rref_speck(&G_expanded, compressed);

        for(int i=0; i<K; i++){
            for(int j=0; j<N-K; j++){
                if(G_expanded.values[i][j] != G.values[i][j]){
                    printf("ERROR IN COMPRESSION/DECOMPRESSION!\n");
                }
            }
        }

        // Then, test expansion from non systematic part to full G
        
        generator_mat_t tmp_full_G;

        for(int i=0; i<K; i++){
            memset(tmp_full_G.values[i],0,sizeof(FQ_ELEM)*N_pad);
        }

        generator_rref_expand(&tmp_full_G, &G);

        for(int i=0; i<K; i++){
            for(int j=0; j<K; j++){
                if ( i!=j && tmp_full_G.values[i][j] != 0){
                    printf("ERROR IN EXPANSION: NO IDENTITY!\n");
                } 
                if ( i==j && tmp_full_G.values[i][j] != 1){
                    printf("ERROR IN EXPANSION: NO IDENTITY!\n");
                }
            }
        }

        for(int i=0; i<K; i++){
            for(int j=0; j<N-K; j++){
                if(tmp_full_G.values[i][j+K] != G.values[i][j]){
                    printf("ERROR IN EXPANSION!\n");
                }
            }
        }

        FQ_ELEM row_by_col;
        for(uint16_t i=0; i < K; i++){
            for(uint16_t j=0; j < K; j++){
                row_by_col = 0;
                for(uint16_t k=0; k<N-K; k++){
                    row_by_col = fq_add(row_by_col,fq_mul(G.values[i][k],G.values[j][k]));
                }
                if(row_by_col != 0 && i!=j){
                    printf("Some rows %u and %u are not orthogonal!\n", i,j);
                    printf("Row by col = %u\n", row_by_col);
                    inner_prod(&row_by_col,G.values[i],G.values[j]);
                    printf("Row by col = %u\n", row_by_col);
                }else if(row_by_col != Q-1 && i==j){
                    printf("Row %u is not self-antiorthogonal\n", i);
                    printf("Row by col = %u\n", row_by_col);
                    inner_prod(&row_by_col,G.values[i],G.values[j]);
                    printf("Row by col = %u\n", row_by_col);
                }
            }
        }
         
        // Now permutation sampling and application
        unsigned char private_permutation_seed[PRIVATE_KEY_SEED_LENGTH_BYTES];
        csprng_randombytes(private_permutation_seed,
                            PRIVATE_KEY_SEED_LENGTH_BYTES,
                            &sk_shake_state);

        permutation_t private_perm;
        permutation_sample_prikey(&private_perm, private_permutation_seed);

        uint8_t histogram[N] = {0};
        
        for(int i=0; i<N; i++){
            histogram[private_perm.values[i]] += 1;
        }

        for(int i=0; i<N; i++){
           if(histogram[i] != 1){
               printf("ERROR IN PERMUTATION SAMPLING!");
           }
        }

        generator_mat_t result_G;
        for(int i=0; i<K; i++){
            //memset(result_G.values[i],0,sizeof(FQ_ELEM)*N_pad);
        }
        permute_generator(&result_G, &tmp_full_G, &private_perm);

        for(int i=0; i<K; i++){
            for(int j=0; j<N; j++){
                if(result_G.values[i][j] != tmp_full_G.values[i][private_perm.values[j]]){
                    printf("ERROR IN PERMUTATION APPLICATION!\n");
                }
            }
        }

        
        for(uint16_t i=0; i < K; i++){
            for(uint16_t j=0; j < K; j++){
                row_by_col = 0;
                for(uint16_t k=0; k<N; k++){
                    row_by_col = fq_add(row_by_col,fq_mul(result_G.values[i][k],result_G.values[j][k]));
                }
                if(row_by_col != 0){
                    printf("In result G rows %u and %u are not orthogonal!\n", i,j);
                    printf("Row by col = %u\n", row_by_col);
                }
            }
        }


        uint8_t is_pivot_column[N_pad];
        memset(is_pivot_column, 0, sizeof(is_pivot_column));
        generator_RREF_speck(&result_G, is_pivot_column);

        for(int i=0; i<K; i++){
            for(int j=0; j<K; j++){
                if ( i!=j && result_G.values[i][j] != 0){
                    printf("ERROR IN RREF: NO IDENTITY!\n");
                } 
                if ( i==j && result_G.values[i][j] != 1){
                    printf("ERROR IN RREF: NO IDENTITY!\n");
                }
            }
        }

        for(uint16_t i=0; i < K; i++){
            for(uint16_t j=0; j < K; j++){
                row_by_col = 0;
                for(uint16_t k=0; k<N; k++){
                    row_by_col = fq_add(row_by_col,fq_mul(result_G.values[i][k],result_G.values[j][k]));
                }
                if(row_by_col != 0){
                    printf("In result G rows %u and %u are not orthogonal!\n", i,j);
                    printf("Row by col = %u\n", row_by_col);
                    break;
                }
            }
        }

        permutation_t private_rref_perm;
        generate_rref_perm(&private_rref_perm, is_pivot_column);

        
        // The final permutation is a combination of the sampled permutation and the one that sends IS to the left
        uint16_t secret_permutation[N];
        for(POSITION_T j = 0; j<N; j++){
            secret_permutation[j] = private_perm.values[private_rref_perm.values[j]];
        }

        // Now we need test that the result is an actual permutation
        // We sample a codeword from G first
        FQ_ELEM u[K_pad] = {0};
        rand_range_q_elements(u, K);
        FQ_ELEM c[N_pad] = {0};
        memcpy(c, u, sizeof(FQ_ELEM)*(K));
        row_mat_mult(c+K,u,K,N_K_pad,G.values,K,N-K);

        // Now we permute it using the secret permutation
        FQ_ELEM permuted_c[N_pad] = {0};
        for(POSITION_T j = 0; j<N; j++){
            permuted_c[j] = c[secret_permutation[j]];
            //permuted_c[secret_permutation[j]] = c[j];
        }

        // Now we should create the H' for G'
        // We just need to transpose and negate A to create M
        // A is k times n-k => M is n-k times k
        
        FQ_ELEM M[K][N_K_pad] = {0};
        generator_rref_compact_speck(M,&result_G,is_pivot_column); 

        // Is M antiorthogonal?
        for(uint16_t i=0; i < K; i++){
            for(uint16_t j=0; j < K; j++){
                row_by_col = 0;
                for(uint16_t k=0; k<N-K; k++){
                    row_by_col = fq_add(row_by_col,fq_mul(M[i][k],M[j][k]));
                }
                if(row_by_col != 0 && i!=j){
                    printf("Some rows %u and %u are not orthogonal!\n", i,j);
                    printf("Row by col = %u\n", row_by_col);
                    inner_prod(&row_by_col,M[i],M[j]);
                    printf("Row by col = %u\n", row_by_col);
                    break;
                }else if(row_by_col != Q-1 && i==j){
                    printf("Row %u is not self-antiorthogonal\n", i);
                    printf("Row by col = %u\n", row_by_col);
                    inner_prod(&row_by_col,M[i],M[j]);
                    printf("Row by col = %u\n", row_by_col);
                    break;
                }
            }
        }

        for(uint16_t i=0; i<K; i++){
           row_mul(M[i],M[i],Q-1);
        }

        // Is M antiorthogonal?
        for(uint16_t i=0; i < K; i++){
            for(uint16_t j=0; j < K; j++){
                row_by_col = 0;
                for(uint16_t k=0; k<N-K; k++){
                    row_by_col = fq_add(row_by_col,fq_mul(M[i][k],M[j][k]));
                }
                if(row_by_col != 0 && i!=j){
                    printf("Some rows %u and %u are not orthogonal!\n", i,j);
                    printf("Row by col = %u\n", row_by_col);
                    inner_prod(&row_by_col,M[i],M[j]);
                    printf("Row by col = %u\n", row_by_col);
                    break;
                }else if(row_by_col != Q-1 && i==j){
                    printf("Row %u is not self-antiorthogonal\n", i);
                    printf("Row by col = %u\n", row_by_col);
                    inner_prod(&row_by_col,M[i],M[j]);
                    printf("Row by col = %u\n", row_by_col);
                    break;
                }
            }
        }


        FQ_ELEM c1[K_pad] = {0};
        memcpy(c1, permuted_c, sizeof(FQ_ELEM)*(K));

        FQ_ELEM c1M[N_K_pad] = {0};
        row_mat_mult(c1M,c1,K,N_K_pad,M,K,N-K);

        for(int i=0; i<N-K; i++){
            if (((c1M[i] + permuted_c[i+K]) % Q) != 0){
                printf("Error in codeword reconstruction from information set\n");
            }
        }
    }
    
}

void test_speck_keygen(){
    speck_prikey_t SK;
    speck_pubkey_t PK;

    generator_mat_t result_GS[NUM_KEYPAIRS-1];

    randombytes(SK.sk_seed, PRIVATE_KEY_SEED_LENGTH_BYTES);

    /* expanding it onto private seeds */
    SHAKE_STATE_STRUCT sk_shake_state;
    initialize_csprng(&sk_shake_state, SK.sk_seed, PRIVATE_KEY_SEED_LENGTH_BYTES);
    /* Generating public code G_0 */
    #ifdef SPECK_RESAMPLE_G
        csprng_randombytes(PK.G_0_seed, SEED_LENGTH_BYTES, &sk_shake_state);
    #else
        unsigned char G_0_seed[SEED_LENGTH_BYTES];
        csprng_randombytes(G_0_seed, SEED_LENGTH_BYTES, &sk_shake_state);
    #endif

    rref_generator_mat_t G0_rref;
    for(int i = 0; i<K; i++){
        //memset(G0_rref.values[i],0,sizeof(uint16_t)*N_K_pad);
    }
    #ifdef SPECK_RESAMPLE_G
        sample_generator(&G0_rref, PK.G_0_seed);
    #endif
    #ifdef SPECK_COMPRESS_G
        sample_generator(&G0_rref, G_0_seed);
        compress_rref_speck_non_IS(PK.G_0_rref,&G0_rref);
    #endif
    #ifdef SPECK_FULL_G
        sample_generator(&G0_rref, G_0_seed);
        //memcpy(PK.G_0_rref,G0_rref.values,sizeof(generator_mat_t));
        memcpy(PK.G_0_rref,G0_rref.values,sizeof(uint16_t)*K*N_K_pad);
    #endif
 
    generator_mat_t tmp_full_G;
    generator_rref_expand(&tmp_full_G, &G0_rref);

    /* The first private key monomial is an ID matrix, no need for random
     * generation, hence NUM_KEYPAIRS-1 */
    unsigned char private_permutation_seeds[NUM_KEYPAIRS - 1][PRIVATE_KEY_SEED_LENGTH_BYTES];
    for (uint32_t i = 0; i < NUM_KEYPAIRS - 1; i++) {
        csprng_randombytes(private_permutation_seeds[i],
                           PRIVATE_KEY_SEED_LENGTH_BYTES,
                           &sk_shake_state);
    }

    /* note that the first "keypair" is just the public generator G_0, stored
     * as a seed and the identity matrix (not stored) */
    for (uint32_t i = 0; i < NUM_KEYPAIRS - 1; i++) {
        uint8_t is_pivot_column[N_pad];
        /* expand inverse monomial from seed */
        permutation_t private_perm;
        permutation_sample_prikey(&private_perm, private_permutation_seeds[i]);

        generator_mat_t result_G;
        for(int j = 0; j<K; j++){
            //memset(result_G.values[j],0,sizeof(uint16_t)*N_pad);
        }

            for(int j = 0; j<K; j++){
                for(uint16_t k = N; k<N_pad; k++){
                    if(result_G.values[j][k] != 0){
                        printf("%u \n",result_G.values[j][k]);
                    }
                }
            }


        permute_generator(&result_G, &tmp_full_G, &private_perm);


            for(int j = 0; j<K; j++){
                for(uint16_t k = N; k<N_pad; k++){
                    if(result_G.values[j][k] != 0){
                        printf("%u \n",result_G.values[j][k]);
                    }
                }
            }

        memset(is_pivot_column, 0, sizeof(is_pivot_column));
        generator_RREF_speck(&result_G, is_pivot_column);

            for(int j = 0; j<K; j++){
                for(uint16_t k = N; k<N_pad; k++){
                    if(result_G.values[j][k] != 0){
                        printf("%u \n",result_G.values[j][k]);
                    }
                }
            }

        permutation_t private_rref_perm;
        generate_rref_perm(&private_rref_perm, is_pivot_column);

        for(POSITION_T j = 0; j<N; j++){
            SK.permutations[i][j] = private_perm.values[private_rref_perm.values[j]];
        }

            for(int j = 0; j<K; j++){
                for(uint16_t k = N; k<N_pad; k++){
                    if(result_G.values[j][k] != 0){
                        printf("%u \n",result_G.values[j][k]);
                    }
                }
            }


        #ifdef SPECK_COMPRESS_GP
            compress_rref_speck(PK.SF_G[i],&result_G,is_pivot_column);
        #else
            generator_rref_compact_speck(PK.SF_G[i],&result_G,is_pivot_column);
        #endif
        printf("%u\n",i);

        result_GS[i] = result_G;
            for(int j = 0; j<K; j++){
                for(uint16_t k = N; k<N_pad; k++){
                    if(result_GS[i].values[j][k] != 0){
                        printf("%u \n",result_GS[i].values[j][k]);
                    }
                }
            }
    }

        rref_generator_mat_t GP_rref;

        // Check keys are well formed
        
        // Check all GP's !
        for(int key = 0; key < NUM_KEYPAIRS -1; key ++){
            #ifdef SPECK_COMPRESS_GP
            expand_to_rref_speck(&GP_rref,PK.SF_G[key]);
            #else
            memcpy(GP_rref.values,PK.SF_G[key],sizeof(uint16_t)*K*N_K_pad);
            #endif
            for(int i = 0; i<K; i++){
                for(uint16_t j = 0; j<N-K; j++){
                    if(GP_rref.values[i][j] != result_GS[key].values[i][j+K]){
                        printf("%u != %u \n",GP_rref.values[i][j],result_GS[key].values[i][j+K]);
                        printf("%u == %u \n",GP_rref.values[i][j],result_GS[key].values[i][j+K]);
                    }
                }
            }
        }

            for(int i = 0; i<NUM_KEYPAIRS-1; i++){
                for(int j = 0; j<K; j++){
                    for(uint16_t k = N; k<N_pad; k++){
                        if(result_GS[i].values[j][k] != 0){
                            printf("%u \n",result_GS[i].values[j][k]);
                        }
                    }
                }
            }

}


#define NUM_TEST_ITERATIONS 10
int main(int argc, char* argv[]){
    (void)argc;
    (void)argv;
    SPECK_sign_verify_test_KAT();
    printf("Done.\n");
    return 0;
}
