#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
#include "fq_arith.h"
#include "permutation.h"
#include "codes.h"
#include "SPECK.h"
#include "parameters.h"
#include "rng.h"
#include "api.h"
#include "sort.h"
#include "transpose.h"

#define GRN "\e[0;32m"
#define WHT "\e[0;37m"

#ifdef N_pad
#define NN N_pad
#else
#define NN N
#endif

/// taken from the nist package.
void fprintBstr(FILE *fp, char *S, unsigned char *A, unsigned long long L) {
    unsigned long long  i;
    fprintf(fp, "%s", S);
    for ( i=0; i<L; i++ )
        fprintf(fp, "%02X", A[i]);
    if ( L == 0 )
        fprintf(fp, "00");
    fprintf(fp, "\n");
}

/* Exhaustive testing of inverses mod Q */
void inverse_mod_tester(void){
    uint32_t value[Q-1];
    uint32_t inverse[Q-1];
    for(uint32_t i=1; i <= Q-1; i++){
        value[i-1] = i;
        inverse[i-1] = fq_inv(i);
    }
    int all_ok = 1;
    for(uint32_t i=1; i <= Q-1; i++){
        if((value[i-1]*inverse[i-1]) % Q !=1){
           printf("%u*%u=%u\n",
                  value[i-1],
                  inverse[i-1],
                  (value[i-1]*inverse[i-1])%Q);
           all_ok = 0;
        }
    }
    if (all_ok){
        puts("All inverses on F_q ok");
    }
}


/*
 *
 */
void info(void){
    fprintf(stderr,"Code parameters: n= %d, k= %d, q=%d\n", N,K,Q);
    fprintf(stderr,"num. keypairs = %d\n",NUM_KEYPAIRS);
    fprintf(stderr,"Fixed weight challenge vector: %d rounds, weight %d \n",T,W);
    fprintf(stderr,"Private key: %luB\n", sizeof(speck_prikey_t));
    fprintf(stderr,"Public key %luB, %.3f kiB\n", sizeof(speck_pubkey_t), ((float) sizeof(speck_pubkey_t))/1024);
    fprintf(stderr,"Signature: %luB, %.3f kiB\n", sizeof(speck_sign_t), ((float) sizeof(speck_sign_t))/1024);

}


#define NUMBER_OF_TESTS 10
#define MLEN 160

int SPECK_sign_verify_test_KAT(void) {
    //unsigned char m[] = {0xD8,0x1C,0x4D,0x8D,0x73,0x4F,0xCB,0xFB,0xEA,0xDE,0x3D,0x3F,0x8A,0x03,0x9F,0xAA,0x2A,0x2C,0x99,0x57,0xE8,0x35,0xAD,0x55,0xB2,0x2E,0x75,0xBF,0x57,0xBB,0x55,0x6A,0xC8};
	//uint8_t seed[] = {0x06,0x15,0x50,0x23,0x4D,0x15,0x8C,0x5E,0xC9,0x55,0x95,0xFE,0x04,0xEF,0x7A,0x25,0x76,0x7F,0x2E,0x24,0xCC,0x2B,0xC4,0x79,0xD0,0x9D,0x86,0xDC,0x9A,0xBC,0xFD,0xE7,0x05,0x6A,0x8C,0x26,0x6F,0x9E,0xF9,0x7E,0xD0,0x85,0x41,0xDB,0xD2,0xE1,0xFF,0xA1};
    // uint8_t seed[] = {0xF1,0x90,0x2A,0x78,0x15,0xF3,0x7B,0xC7,0xF5,0x80,0x2D,0x8C,0xBC,0xE5,0xB4,0x8D,0x82,0xEB,0x85,0x69,0x17,0x18,0x06,0x2B,0xFB,0x84,0xD8,0xC0,0x6A,0xA4,0x1D,0x6E,0x90,0x39,0xB0,0xA1,0x07,0x24,0x5D,0xAF,0xA4,0xEC,0x10,0x9A,0x57,0x33,0x29,0x14};
    // uint8_t m[] = {};
    uint8_t seed[] ={0x83,0xC6,0x53,0x70,0x8F,0xAF,0x3E,0x5F,0x6F,0xBC,0x9D,0xFB,0xE6,0xFB,0x5E,0x83,0xE5,0x72,0xA7,0x68,0x86,0x45,0xD7,0x5D,0x2C,0x48,0x35,0xB2,0x86,0x95,0xDE,0xA4,0xBD,0x70,0x93,0x74,0x0D,0x0F,0xF4,0x32,0x37,0x35,0x4E,0xAD,0x1C,0x97,0x8B,0xC2};
    uint8_t m[] ={};

    //for (uint32_t i=0; i<48; i++) {
    //    seed[i] = i;
	//}

    // init_randombytes(seed, 48);
    initialize_csprng(&platform_csprng_state,
                      (const unsigned char *)seed,
                      48);

    const uint32_t mlen = sizeof(m);
    unsigned long long smlen = 0, mlen1;
    unsigned char *m1 = (unsigned char *)calloc(mlen+CRYPTO_BYTES, sizeof(unsigned char));
    unsigned char *sm = (unsigned char *)calloc(mlen+CRYPTO_BYTES, sizeof(unsigned char));
    unsigned char pk[CRYPTO_PUBLICKEYBYTES] = {0}, sk[CRYPTO_SECRETKEYBYTES] = {0};

    int ret_val;
    if ((ret_val = crypto_sign_keypair(pk, sk)) != 0) {
        printf("crypto_sign_keypair returned <%d>\n", ret_val);
        return -1;
    }
    fprintBstr(stdout, "pk = ", pk, CRYPTO_PUBLICKEYBYTES);
    fprintBstr(stdout, "sk = ", sk, CRYPTO_SECRETKEYBYTES);

    if ( (ret_val = crypto_sign(sm, &smlen, m, mlen, sk, pk)) != 0) {
        printf("crypto_sign returned <%d>\n", ret_val);
        return -1;
    }

    fprintBstr(stdout, "sm = ", sm, smlen);
    if ( (ret_val = crypto_sign_open(m1, &mlen1, sm, smlen, pk)) != 0) {
        printf("crypto_sign_open returned <%d>\n", ret_val);
        return -1;
    }

    free(m1);
    free(sm);
    printf("all good\n");
    return 0;
}

void test_sample_antiorthogonal(){
    unsigned char compressed_sk[PRIVATE_KEY_SEED_LENGTH_BYTES];
    randombytes(compressed_sk, PRIVATE_KEY_SEED_LENGTH_BYTES);
    SHAKE_STATE_STRUCT sk_shake_state;
    initialize_csprng(&sk_shake_state, compressed_sk, PRIVATE_KEY_SEED_LENGTH_BYTES);
    unsigned char G_seed[SEED_LENGTH_BYTES];
    csprng_randombytes(G_seed, SEED_LENGTH_BYTES, &sk_shake_state);

    FQ_ELEM A[K][K_pad];
    antiorthogonal_sample(A,G_seed);

    FQ_ELEM row_by_col;
    for(uint8_t i=0; i < K; i++){
        for(uint8_t j=0; j < K; j++){
            row_by_col = 0;
            for(uint8_t k=0; k<K; k++){
                row_by_col = fq_add(row_by_col,fq_mul(A[i][k],A[j][k]));
            }
            if(row_by_col != 0 && i!=j){
                printf("FAIL! Some row is not orthogonal!\n");
            }else if(row_by_col != 126 && i==j){
                printf("FAIL! Some row is not self-antiorthogonal\n");
            }
        }
    }
}

void test_fq_operations(){
    FQ_ELEM c[K];
    for(int i=0; i<10; i++){
        rand_range_q_elements(c,K);
        printf("Val: %i\n", c[5]);
        printf("Mult by 2: %i\n", fq_mul(c[5],2));
        printf("Adding 1: %i\n", fq_add(c[5],1));


        printf("Codeword: [");
        for(uint8_t i=0; i<K; i++){
            printf(" %i, ", c[i]);
        }
        printf("]\n");
        FQ_ELEM in_prod;
        inner_prod(&in_prod,c);
        printf("Inner prod: %i\n", in_prod);
        anti_normalize(c);
        inner_prod(&in_prod,c);
        printf("Inner prod after normalization: %i\n", in_prod);
    }
}

void test_permutation(){
   POSITION_T permutation[N];

   for(int i = 0; i<N; i++){
       permutation[i] = i;
    }

    unsigned char compressed_sk[PRIVATE_KEY_SEED_LENGTH_BYTES];
    randombytes(compressed_sk, PRIVATE_KEY_SEED_LENGTH_BYTES);
    SHAKE_STATE_STRUCT sk_shake_state;
    initialize_csprng(&sk_shake_state, compressed_sk, PRIVATE_KEY_SEED_LENGTH_BYTES);
    yt_shuffle_state_limit(&sk_shake_state, permutation, N);
    printf("[");
    for(int i = 0; i<N; i++){
        printf(" %i ",permutation[i]);
     }
    printf("]\n");
}


void test_anti_normalize(){
    unsigned char compressed_sk[PRIVATE_KEY_SEED_LENGTH_BYTES];
    randombytes(compressed_sk, PRIVATE_KEY_SEED_LENGTH_BYTES);
    SHAKE_STATE_STRUCT sk_shake_state;
    initialize_csprng(&sk_shake_state, compressed_sk, PRIVATE_KEY_SEED_LENGTH_BYTES);
    unsigned char G_seed[SEED_LENGTH_BYTES];
    csprng_randombytes(G_seed, SEED_LENGTH_BYTES, &sk_shake_state);
    FQ_ELEM c[K];
    int counter = 0;
    int samples = 1000;
    for(int i=0; i<samples; i++){
        rand_range_q_state_elements(&sk_shake_state, c, K);
        FQ_ELEM i_prod;
        inner_prod(&i_prod,c);
        anti_normalize(c);
        inner_prod(&i_prod,c);
        if(i_prod == 126) counter++;
    }
    printf("Counter antinormalization: %i/%i\n",counter,samples); // -> 50%
}

int check_antiorthogonal(const FQ_ELEM M[K][K]){
    //Check decompressed G antiorthogonality
    FQ_ELEM row_by_col;
    for(uint8_t i=0; i < K; i++){
        for(uint8_t j=0; j < K; j++){
            row_by_col = 0;
            for(uint8_t k=0; k<K; k++){
                row_by_col = fq_add(row_by_col,fq_mul(M[i][k],M[j][k]));
            }
            if(row_by_col != 0 && i!=j){
                printf("FAIL! Some row is not orthogonal!\n");
                return 1;
            }else if(row_by_col != 126 && i==j){
                printf("FAIL! Some row is not self-antiorthogonal\n");
                return 1;
            }
        }
    }
    return 0;
}


void test_speck_keygen(){
    printf("KEYPAIRS: %i\n", NUM_KEYPAIRS);
    for(int sample=0; sample<100; sample++){
        speck_prikey_t sk;
        speck_pubkey_t pk;
        SPECK_keygen(&sk,&pk);

        // Get public G
        rref_generator_mat_t G_rref;
#ifdef SPECK_RESAMPLE_G
        generator_sample(&G_rref, pk.G_0_seed);
#endif
#ifdef SPECK_COMPRESS_G 
        expand_to_rref_speck(&G_rref,pk.G_0_rref);
#endif
        //check_antiorthogonal(G_rref.values);

        // Sample codeword from G 
        FQ_ELEM c[N] = {0};
        sample_codeword_rref(c,G_rref);

        for(int key = 0; key < NUM_KEYPAIRS-1; key++){
            // Get public GP
#ifdef SPECK_COMPRESS_GP
            rref_generator_mat_t GP_rref;
            expand_to_rref_speck(&GP_rref,pk.SF_G[key]);
#endif
            //check_antiorthogonal(GP_rref.values);

            // Permute the codeword (c -> cp)
            FQ_ELEM cp[N] = {0};
            for(int i=0; i<N; i++){
                cp[i] = c[sk.permutations[key][i]];
            }

            // Check cp belongs to GP
            // (No need to compute H for now, just use G as parity-check)
            // cp = [cp1 cp2], cp*GP.T = [cp1 cp2*A.T]
            FQ_ELEM cp2AT[K] = {0};

            for(uint8_t i=0; i<K; i++){
                cp2AT[i] = 0; 
                for(uint8_t j=0; j<K; j++){
#ifdef SPECK_COMPRESS_GP
                    cp2AT[i] = fq_add(cp2AT[i],
                            fq_mul(cp[j+K],GP_rref.values[i][j]));
#else
                    cp2AT[i] = fq_add(cp2AT[i],
                            fq_mul(cp[j+K],pk.SF_G[key][i][j]));
#endif
                }
            }

            for(uint8_t i=0; i<K; i++){
                if(cp2AT[i] != fq_sub(0,cp[i])){
                    printf("Codeword doesn't belong to code, cp2*A.T[%i] != -cp1[%i]\n",i,i);
                }
            }
        }
    } 
}

void test_speck_round(){
    speck_prikey_t sk;
    speck_pubkey_t pk;
    SPECK_keygen(&sk,&pk);

    // Get public G
    rref_generator_mat_t G_rref;
#ifdef SPECK_RESAMPLE_G
    generator_sample(&G_rref, pk.G_0_seed);
#endif
#ifdef SPECK_COMPRESS_G 
    expand_to_rref_speck(&G_rref,pk.G_0_rref);
#endif
    //check_antiorthogonal(G_rref.values);

    // Sample codeword from G 
    FQ_ELEM c[N] = {0};
    sample_codeword_rref(c,G_rref);

    // Sample a random permutation
    permutation_t p_star;
    unsigned char p_star_seed[PRIVATE_KEY_SEED_LENGTH_BYTES];
    randombytes(p_star_seed,PRIVATE_KEY_SEED_LENGTH_BYTES);
    permutation_sample_prikey(&p_star, p_star_seed);
    

    for(int key = 0; key < NUM_KEYPAIRS-1; key++){
        // Get public GP
#ifdef SPECK_COMPRESS_GP
        rref_generator_mat_t GP_rref;
        expand_to_rref_speck(&GP_rref,pk.SF_G[key]);
#endif
        //check_antiorthogonal(GP_rref.values);

        // Permute the codeword (c -> cp)
        FQ_ELEM cp[N];
        permutation_t p;
        memcpy(p.values,sk.permutations[key],sizeof(POSITION_T)*N);
        permute_codeword(cp,c,&p);

        // Check cp belongs to GP
        // (No need to compute H for now, just use G as parity-check)
        // cp = [cp1 cp2], cp*GP.T = [cp1 cp2*A.T]
        FQ_ELEM cp2AT[K] = {0};

        for(uint8_t i=0; i<K; i++){
            cp2AT[i] = 0; 
            for(uint8_t j=0; j<K; j++){
#ifdef SPECK_COMPRESS_GP
                cp2AT[i] = fq_add(cp2AT[i],
                        fq_mul(cp[j+K],GP_rref.values[i][j]));
#else
                cp2AT[i] = fq_add(cp2AT[i],
                        fq_mul(cp[j+K],pk.SF_G[key][i][j]));
#endif
            }
        }

        for(uint8_t i=0; i<K; i++){
            if(cp2AT[i] != fq_sub(0,cp[i])){
                printf("Codeword doesn't belong to code, cp2*A.T[%i] != -cp1[%i]\n",i,i);
            }
        }
    }
}

void test_sorting(){
    speck_prikey_t sk;
    speck_pubkey_t pk;
    SPECK_keygen(&sk,&pk);

    // Get public G
    rref_generator_mat_t G_rref;
#ifdef SPECK_RESAMPLE_G
    generator_sample(&G_rref, pk.G_0_seed);
#endif
#ifdef SPECK_COMPRESS_G 
    expand_to_rref_speck(&G_rref,pk.G_0_rref);
#endif
    //check_antiorthogonal(G_rref.values);

    // Sample codeword from G 
    FQ_ELEM c[N] = {0};
    sample_codeword_rref(c,G_rref);

    // Order by counting sort
    FQ_ELEM x1[N] = {0};
    permutation_t ordering_perm;
    counting_sort_perm(x1,c,N,&ordering_perm);

    // Order by permutation 
    FQ_ELEM x2[N] = {0};
    permute_codeword(x2,c,&ordering_perm);

    // Compare
    for(int i=0; i<N; i++){
        if(x1[i] != x2[i]) printf("ERROR IN SORTING");
    }
}

void test_histogram(){
    speck_prikey_t sk;
    speck_pubkey_t pk;
    SPECK_keygen(&sk,&pk);

    // Get public G
    rref_generator_mat_t G_rref;
#ifdef SPECK_RESAMPLE_G
    generator_sample(&G_rref, pk.G_0_seed);
#endif
#ifdef SPECK_COMPRESS_G 
    expand_to_rref_speck(&G_rref,pk.G_0_rref);
#endif
    //check_antiorthogonal(G_rref.values);

    // Sample codeword from G 
    FQ_ELEM c[N] = {0};
    sample_codeword_rref(c,G_rref);

    uint8_t x[Q];
    histogram(x,c,N);

    printf("Histogram: ");
    for (int i=0; i<Q; i++) {
        printf(" %i ",x[i]);
    }
    printf("\n");
}

int SPECK_sign_verify_test_multiple(void){
    unsigned long long smlen = 0, mlen1;
    unsigned char *m  = (unsigned char *)calloc(MLEN, sizeof(unsigned char));
    unsigned char *sm = (unsigned char *)calloc(MLEN+CRYPTO_BYTES, sizeof(unsigned char));
    unsigned char       pk[CRYPTO_PUBLICKEYBYTES] = {0}, sk[CRYPTO_SECRETKEYBYTES] = {0};

    int ret = 0;
    for (size_t i = 0; i < NUMBER_OF_TESTS; ++i) {
        const uint32_t msg_len = (uint32_t)rand() % MLEN;
        randombytes(m, msg_len);

        int ret_val;
        if ((ret_val = crypto_sign_keypair(pk, sk)) != 0) {
            printf("crypto_sign_keypair returned <%d>\n", ret_val);
            return -1;
        }

        fprintBstr(stdout, "pk = ", pk, CRYPTO_PUBLICKEYBYTES);
        fprintBstr(stdout, "sk = ", sk, CRYPTO_SECRETKEYBYTES);

        if ( (ret_val = crypto_sign(sm, &smlen, m, msg_len, sk, pk)) != 0) {
            printf("crypto_sign returned <%d>\n", ret_val);
            return -1;
        }
        fprintBstr(stdout, "sm = ", sm, smlen);
        if ( (ret_val = crypto_sign_open(m, &mlen1, sm, smlen, pk)) != 0) {
            printf("crypto_sign_open returned <%d>\n", ret_val);
            return -1;
        }

        if(mlen1 != msg_len) {
            printf("crypto_sign_open wrong length\n");
            return -1;
        }
        printf("OK: %zu\n", i);

        ret |= ret_val;
    }

    return ret;
}

void test_c1s_compression(){
    speck_prikey_t sk;
    speck_pubkey_t pk;
    SPECK_keygen(&sk,&pk);

    FQ_ELEM u[K];
    FQ_ELEM c1s[W][K_pad];
    for(int i=0; i<W; i++){
        rand_range_q_elements(u,K);
        for(int j=0; j<W; j++){
            c1s[i][j] = u[j];
        }
    }

    FQ_ELEM compressed_c1s[SPECK_C1S_PACKEDBYTES];
    compress_c1s(compressed_c1s,c1s);

    FQ_ELEM recovered_c1s[W][K_pad];
    expand_c1s(recovered_c1s,compressed_c1s);
    for(int i=0; i<W; i++){
        for(int j=0; j<W; j++){
            if(c1s[i][j] != recovered_c1s[i][j]){
                printf("c1s do not coincide!\n");
            }
        }
    }

}

void test_transpose(void){

    FQ_ELEM G[K_pad][K_pad] __attribute__((aligned(32))) = {0};
    for(int i=0; i<K; i++){
        rand_range_q_elements(G[i],K);
    }

    FQ_ELEM G_T[K_pad][K_pad] __attribute__((aligned(32))) = {0};

    matrix_transpose_opt((uint8_t *)G_T,(uint8_t *)G,40,40);

    for(int i=0; i<20; i++){
        for(int j=0; j<20; j++){
            if(G_T[i][j] != G[j][i]){
                printf("You didn't transpose little fucker\n");
            }
        }
    }

    
    /*
    printf("G first row");
    printf("[");
    for(int i=0; i<K; i++){
        printf("%d, ", G[0][i]);
    }
    printf("]\n");
    printf("G_T first column");
    for(int i=0; i<K; i++){
        printf("%d, ", G_T[i][0]);
    }
    printf("]\n");
    */

}

void test_row_mat_mult(void){
    FQ_ELEM out_avx2[K_pad];
    FQ_ELEM out_normal[K_pad] = {0};
    FQ_ELEM c[K_pad] = {0};
    rand_range_q_elements(c,K);
    FQ_ELEM G[K][K_pad] = {0};
    for(int i=0; i<K; i++){
        rand_range_q_elements(G[i],K);
    }
    row_mat_mult(out_avx2,c,G,126,126);
    row_mat_mult_old(out_normal,c,G,126,126);

    for(int i=0; i<K; i++){
        if(out_avx2[i] != out_normal[i]) printf("ERROR ON ROW MAT MULT! %i %u != %u\n", i, out_avx2[i], out_normal[i]);
    }

    //memset(out_normal,0,sizeof(FQ_ELEM)*(NEXT_MULTIPLE(10, 32)-10));
    memset(out_avx2,0,sizeof(FQ_ELEM)*K_pad);
    memset(out_normal,0,sizeof(FQ_ELEM)*K_pad);

    row_mat_mult(out_avx2,c,G,10,10);
    memset(out_avx2+10,0,sizeof(FQ_ELEM)*(NEXT_MULTIPLE(10, 32)-10));
    row_mat_mult_old(out_normal,c,G,10,10);

    //for(int i=0; i<K; i++){
    //    if(out_avx2[i] != out_normal[i]){
    //        printf("Elem %d: ERROR ON ROW MAT MULT!\n",i);
    //    }
    //}
}

#define NUM_TEST_ITERATIONS 10
#define USE_AVX
int main(int argc, char* argv[]){
    (void)argc;
    (void)argv;
    //SPECK_sign_verify_test_KAT();
    test_transpose();
    //SPECK_sign_verify_test_multiple();
    //test_fq_operations();
    //test_row_mat_mult();
    printf("Done, all worked\n");
    return 0;
}
